...
import re
import uuid
from awscrt import io, mqtt, auth, http, awsiot_mqtt_connection_builder
...
ENDPOINT = "a34nag6595lz0u-ats.iot.eu-west-1.amazonaws.com"
#CLIENT_ID = "cimw"
CLIENT_ID = (':'.join(['{:02x}'.format((uuid.getnode() >> ele) & 0xff)
for ele in range(0,8*6,8)][::-1]))
print("The MAC address in formatted way is :", CLIENT_ID)
PATH_TO_CERT = "/etc/accenture/certs/device.pem.crt"
PATH_TO_KEY = "/etc/accenture/certs/private.pem.key"
PATH_TO_ROOT = "/etc/accenture/certs/Amazon-root-CA-1.pem"
#MESSAGE = "Hello World"
TOPIC = "$aws/things/cimw/shadow/update"
#RANGE = 2000
...
mqtt_connection = awsiot_mqtt_connection_builder.mtls_from_path(
            endpoint=ENDPOINT,
...
class Options:
    def __init__(self, args):
        ...
        try:
            opts, args = getopt.getopt(args, 'acdfghilmno:prstTvyC:D:I:M:N:S:V:X',
...
class dstat_cpu(dstat):
    def __init__(self):
        #self.nick  = ( 'CPU-usr', 'CPU-sys', 'CPU-idl', 'CPU-wai', 'CPU-stl', 'MacAddress' )
        self.nick  = ( 'CPU-usr', 'CPU-sys', 'CPU-idl', 'CPU-wai', 'CPU-stl' )
        self.type  = 'p'
        self.width = 10
        self.scale = 34
        self.open('/proc/stat')
...
class dstat_cpu_adv(dstat_cpu):
    def __init__(self):
        self.nick  = ( 'Cpu-usr', 'Cpu-sys', 'Cpu-idl', 'Cpu-wai', 'hiq', 'siq', 'Cpu-stl' )
        self.type  = 'p'
        self.width = 7
        self.scale = 34
        self.open('/proc/stat')
...
class dstat_cpu24(dstat):
    def __init__(self):
        self.nick  = ( 'Cpu-usr', 'Cpu-sys', 'Cpu-idl')
        self.type  = 'p'
        self.width = 7
        self.scale = 34
        self.open('/proc/stat')
...
class dstat_disk24(dstat):
    def __init__(self):
        self.nick = ('Disk-read ', 'Disk-write')
        self.type = 'b'
        self.diskfilter = re.compile('^([hsv]d[a-z]+\d+|cciss/c\d+d\d+p\d+|dm-\d+|md\d+|mmcblk\d+p\d0|VxVM\d+)$')
...
class dstat_mac(dstat):
    def __init__(self):
        self.name = 'mac'
        self.vars = ('MacAddress',)
        self.width = 20
        self.type = 's'
        if op.debug:
            self.width = 13
...
class dstat_mem(dstat):
    def __init__(self):
        ...
        self.nick = ('Mem-used', 'Mem-free', 'Mem-buff', 'Mem-cach')
...
class dstat_proc(dstat):
    def __init__(self):
        self.name = 'procs'
        self.nick = ('proc-run', 'proc-blk', 'proc-new')
...
class dstat_time(dstat):
    ...
    def extract(self):
        if op.debug:
            self.val['date time'] = time.strftime(self.timefmt, time.localtime(starttime)) + ".%03d" % (round(starttime * 1000 % 1000 ))
        else:
            self.val['date time'] = time.strftime(self.timefmt, time.localtime(starttime))
...
def perform(update):
        ...
        if sys.stdout.isatty():
            ...
            if op.header and rows >= 6:
                if oldvislist == vislist or oldvislist != vislist:
                    showheader = True
        ...
        beforeConversion = line + theme['input']
        convertedLine= escape_ansi(beforeConversion)
        MESSAGE = convertedLine
        key = "key"
        #Publishing data
        #data = "{} [{}]".format(MESSAGE, key)
        #message = {"key": MESSAGE}
        message = MESSAGE
        #message = {"message": data}
        mqtt_connection.publish(topic=TOPIC, payload=json.dumps(message), qos=mqtt.QoS.AT_LEAST_ONCE)
        print("\nPublished: '" + json.dumps(message) + "' to the topic: " + "'test/testing'")
        t.sleep(0.1)
        #print("\n+++++++++++++++++++++++++++++++++++++++++++++++++")
        #print("\nJson Format"+convertedLine)
